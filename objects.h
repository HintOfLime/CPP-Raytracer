...
class Primitive {
    public:
        ...
        void init (Material*, Material*, float, float, float);
};
...
class Sphere: public Primitive {
    public:
        Vector3 center;
        float radius;
        Sphere (Vector3, float, Material*, Material*, float, float, float);
        float intersect (Ray&);
...
class Plane: public Primitive {
    public:
        ...
        float width;
        float height;
        Plane (Vector3, Vector3, float, float, Material*, Material*, float, float, float);
        float intersect (Ray&);
        Vector3 getNormal (Vector3);
        Vector3 getCenter() {return this->center;}
        Vector3 getSurfCoords(Vector3);
